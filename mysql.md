...
1. 避免在循环中反复提交操作。因为每次提交都要写一次重做日志。所以，可以一次事务内部做循环，循环结束后，再做提交。

2. 不同的编程语言对于MySQL API，支持的"自动提交"的默认行为是不一致的。MySQL C API默认的提交方式是自动提交；MySQL Python API则默认禁用了自动提交。


### 长事务

对于长事务的处理，建议讲"大事物"的逻辑修改为：批量处理小事务。
...
**复制的作用：** MySQL数据库提供的一种高可用高性能的解决方案。其工作原理分为下列3步骤：

1. 主服务器（master）：把数据更改记录到`二进制日志（binlog）`中。
2. 从服务器（slave）：把主服务器的二进制日志复制到自己的`中继日志（relay log）`中。
3. 从服务器重做*中继日志*中的日志，把更改应用到自己的数据库上，以达到数据的最终一致性。

所以复制的工作原理实际上是：一个完全备份加上二进制日志备份的还原。
复制不完全实时地进行同步，而是`异步实时`。
...
1. I/O线程，负责读取主服务器的二进制日志，并将其保存为中继日志。
2. SQL线程，复制执行中继日志。
